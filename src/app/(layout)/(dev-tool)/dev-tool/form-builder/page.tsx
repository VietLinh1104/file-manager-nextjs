...
export default function FormBuilderPage() {
	...
	.map((bo) => {
		const bindingMap = Object.fromEntries(
			(boFieldMap[bo.id] || []).map((f) => [f.boundTo, f])
		)
...
${bo.fields
	.map((boF) => {
		...
		return "" // optional field không bind thì bỏ qua
	})
	.filter(Boolean)
	.join(",\n")}
		}`
...
${objects}
	...
	console.log("Submitted objects:", { ${usedBOs
		.map((bo) => bo.name.toLowerCase())
		.join(", ")} })
	}
		...
		<DndContext onDragEnd={handleDragEnd}>
			<div className="flex h-screen">
				...
				<div className="flex-1 p-6 overflow-y-auto">
							...
							{formFields.map((f) => (
								...
								>
									<span className="text-sm">
										{f.label} ({f.id}) {f.boundTo ? `→ ${f.boundTo}` : ""}
									</span>
					...
					{generatedCode && (
						<pre className="mt-4 p-4 border rounded bg-muted text-xs overflow-x-auto whitespace-pre-wrap">
							{generatedCode}
						</pre>
					)}
				</div>
				...
				<aside className="w-80 border-l flex flex-col p-4 bg-background">
					<div>
						...
						<div className="flex flex-wrap gap-2">
							{["text", "number", "textarea", "select", "radio", "checkbox", "date", "file"].map(
								(t) => (
							...
							<TabsContent value="props" className="mt-2 space-y-3 flex-1 overflow-y-auto">
								<div>
									...
									<Input
										...
										onChange={(e) =>
											updateField(selectedField.internalKey, { id: e.target.value })
										}
									/>
								</div>
								<div>
									...
									<Input
										...
										onChange={(e) =>
											updateField(selectedField.internalKey, { label: e.target.value })
										}
									/>
								</div>
								<div>
									...
									<Input
										...
										onChange={(e) =>
											updateField(selectedField.internalKey, { placeholder: e.target.value })
										}
									/>
								</div>
							</TabsContent>
