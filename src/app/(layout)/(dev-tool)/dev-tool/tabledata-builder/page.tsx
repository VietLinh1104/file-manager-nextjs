...
export default function TableBuilderPage() {
	...
	const mockRows = Array.from({ length: 5 }).map((_, i) => {
		const fields = selectedBO.fields
			.map((f) => {
				if (f.type === "string") return `${f.name}: "${f.name}_${i + 1}"`
				if (f.type === "number") return `${f.name}: ${i + 1}`
				if (f.type === "boolean") return `${f.name}: ${i % 2 === 0}`
				if (f.type === "date") return `${f.name}: new Date().toISOString()`
				return `${f.name}: null`
			})
			.join(", ")
		return `{ ${fields} }`
